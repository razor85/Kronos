...
void UISettings::tbBrowse_clicked()
{
  ...
  else if (tb == tbCppFilt)
    requestFile(QtYabause::translate("Choose the location of the c++filt executable"), leCppFilt);
}
...
void UISettings::loadSettings()
{
  ...
  leCppFilt->setText(s->value("Debug/CppFilt").toString());
...
void UISettings::saveSettings()
{
  ...
  s->setValue("Debug/CppFilt", leCppFilt->text());
  QtYabause::updateTitle();
}
